...
# $Header: /var/cvsroot/gentoo-x86/net-www/mozilla-firefox/mozilla-firefox-w_mng-0.9.1.ebuild
...
IUSE="java gtk2 ipv6 gnome moznoxft truetype xinerama mng"
...
src_unpack() {
	...
	use mng && ( epatch ${FILESDIR}/firefox-${PV}-mng.patch.bz2 ; autoconf )
}
...
src_compile() {
	...
	# Use included MNG library with MNG_BUILD_MOZ_MNG
	if use mng; then
		myconf="${myconf} --without-system-mng \
						  --enable-image-decoders=default,mng \
						  --enable-mng-type=MNG_BUILD_MOZ_MNG"
	fi
