...
src_compile () {
	...
	myconf="`use_with zlib` `use_with ipv6`"
...
src_install() {
	...
	dodoc *-New alt-irc-faq Authors BUGS ChangeLog Etiquette example.conf \
		...
		m4macros README* RELEASE* SERVICE*
	docinto Juped
...
#	groupadd -g `cat ${T}/group` -o ircd
	enewgroup ircd `cat ${T}/group`
#	useradd -c "IRCd server user" -d /etc/ircd -g `cat ${T}/group` -o -s /bin/false -u `cat ${T}/user` ircd
	enewuser ircd `cat ${T}/user` /bin/false /etc/ircd `cat ${T}/group` # -c "IRCd server user"
}
