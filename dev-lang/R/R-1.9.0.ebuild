...
# $Header: /home/cvsroot/gentoo-x86/dev-lang/R/R-1.9.0.ebuild,v 1.5 2004/05/01 03:47:15 okayama Exp $
...
X11_PATCH="${P}.i18n.x11_mb.20040424.patch"
...
src_compile() {
	...
	econf ${myconf} \
		...
		CPPFLAGS="${MAIN_CFLAGS}" \
		FFLAGS="${CFLAGS}" \
		R_BROWSER="${R_BROWSER}" || die "econf failed"

	emake || die

	#If you want to run check, uncomment this.
	#LANG=C LANGUAGE=C LC_ALL=C make check-all || die "check failed"
