...
# $Header: /home/cvsroot/gentoo-x86/sys-devel/gcc/gcc-3.3.4-r3.ebuild,v 1.2 2004/12/19 15:24:33 okayama Exp $

inherit eutils flag-o-matic libtool gnuconfig versionator toolchain-funcs

IUSE="static nls bootstrap build X multilib gcj gdc fortran objc pic hardened uclibc debug"
...
if use gdc
then
	SRC_URI="${SRC_URI} http://home.earthlink.net/~dvdfrdmn/d/gdc-0.8.tgz"
fi
SRC_URI="${SRC_URI}
...
src_unpack() {
	...
	then
		cd ${S}
		epatch ${FILESDIR}/Makefile.in.gdc.patch
		sed -i -e 's/\(target-libobjc\)/\1 target-phobos/' \
			${S}/configure.in || die "sed failed"
		sed -i -e 's/\(^      { echo ".*time" 1>&2;\) exit 1;/\1/' \
			${S}/boehm-gc/configure || die "sed failed"
		cd ${S}/gcc/
		unpack gdc-0.8.tgz
		epatch d/patch-gcc-$(gcc-version).x
		sed -i -e 's/\(target_libs=\)"target-boehm-gc"/\1target-phobos/' \
			d/config-lang.in || die "sed failed"
		mv d/phobos ../
		cd ../phobos
		ln -sf ../boehm-gc boehm-gc
		sed -i -e \
			"s|\(#! /bin/sh\)|\1\nPATH=${WORKDIR}/build/gcc:${PATH}|" \
			configure   || die "sed failed"
		epatch ${FILESDIR}/phobos-conf.patch
		sed -i -e \
			"s|\(DFLAGS=.*\)|\1\nPATH=${WORKDIR}/build/gcc:${PATH}|" \
	...
	then
		...
		use uclibc && epatch ${FILESDIR}/gcc-uclibc-3.3-loop.patch
	fi
	...
	then
		epatch ${FILESDIR}/gcc333_pre20040408-stack-size.patch
	fi

	if [ "${ARCH}" = "arm" ]
	then
		epatch ${FILESDIR}/gcc333-debian-arm-getoff.patch
		epatch ${FILESDIR}/gcc333-debian-arm-ldm.patch
	fi
	...
	then
		...
		epatch ${FILESDIR}/gcc331-pp-fixup.patch

		EPATCH_OPTS="${EPATCH_OPTS} ${WORKDIR}/protector.dif" \
		epatch ${FILESDIR}/gcc333-ssp-3.3.2_1-fixup.patch
		...
		[ -n "${PATCH_VER}" ] && epatch ${FILESDIR}/gcc-3.3.3-uclibc-add-ssp.patch
	...
	(use pic || use hardened) && epatch ${FILESDIR}/gcc-3.3.3-libiberty-pic.patch

	version_patch ${FILESDIR}/gcc334-gentoo-branding.patch \
	...
	epatch ${FILESDIR}/gcc-3.3.4-spec-env.patch
