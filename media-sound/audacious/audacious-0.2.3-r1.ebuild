...
# $Header: /var/cvsroot/gentoo-x86/media-sound/audacious/audacious-0.2.3.ebuild,v 1.6 2006/04/17 21:02:53 corsair Exp $

IUSE="aac alsa cjk esd flac gnome jack lirc mmx modplug mp3 musepack nls oss sid sndfile vorbis wma"
...
SRC_URI="http://audacious-media-player.org/release/${P}.tgz
	...
	cjk? ( http://www.cc.rim.or.jp/~yaz/${P}_yaz.diff.gz )"
...
DEPEND="${RDEPEND}
	...
	cjk? ( dev-libs/libguess )
	>=dev-util/pkgconfig-0.9.0"

src_unpack() {
	unpack ${P}.tgz gentoo_ice-xmms-0.2.tar.bz2

	if useq cjk; then
		epatch ${DISTDIR}/${P}_yaz.diff.gz
	fi
}
